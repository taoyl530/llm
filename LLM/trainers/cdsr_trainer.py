...
class CDSRTrainer(SeqTrainer):
    ...
    def _create_model(self):
        ...
        # === [修复] 显式构建跨域图 ===
        # 只有在开启 GNN 且模型有构图方法时才调用
        if getattr(self.args, 'use_gnn', False):
            self.logger.info("Initializing Cross-Domain Graph Structure...")
            self.model._build_cross_domain_graph(self.args)
            # 确保边索引被移动到正确的设备 (如果模型里没有自动处理)
            if self.model.edge_u is not None:
                self.model.edge_u = self.model.edge_u.to(self.device)
                self.model.edge_i = self.model.edge_i.to(self.device)
                self.model.user_deg = self.model.user_deg.to(self.device)
                self.model.item_deg = self.model.item_deg.to(self.device)
        # ===========================
    ...
    def eval(self, epoch=0, test=False):
        ...
        if test:
            ...
            # [修复] 测试加载模型后也需要确保图数据在设备上
            if getattr(self.args, 'use_gnn', False):
                 # 如果模型保存时没保存图结构(因为是buffer不是param)，这里需要重新构建或确保存在
                 # 通常 _build_cross_domain_graph 是确定性的，重跑一次无妨
                 if self.model.edge_u is None or self.model.edge_u.device.type == 'cpu':
                     self.model._build_cross_domain_graph(self.args)
