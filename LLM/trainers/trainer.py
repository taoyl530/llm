...
class Trainer(object):
    ...
    def __init__(self, args, logger, writer, device, generator):
        ...
        self.watch_metric = getattr(args, 'watch_metric', 'NDCG@10')
    ...
    def _set_scheduler(self):
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer,
                                                         step_size=getattr(self.args, 'lr_dc_step', 10),
                                                         gamma=getattr(self.args, 'lr_dc', 0.1)
                                                         )
    ...
    def train(self):
        ...
        for epoch in trange(self.start_epoch, self.start_epoch + int(self.args.num_train_epochs), desc="Epoch"):
            ...
            if (epoch % 1) == 0:
                ...
                # append epoch JSON record
                try:
                    if hasattr(self.args, 'log_jsonl_path'):
                        import json
                        epoch_record = {
                            'epoch': int(epoch + 1),
                            'train_loss': float(t) if t is not None else None,
                            'metrics': metric_dict,
                        }
                        with open(self.args.log_jsonl_path, 'a', encoding='utf-8') as f:
                            f.write(json.dumps(epoch_record, ensure_ascii=False) + '\n')
                except Exception as e:
                    self.logger.info(f"Failed to write JSONL: {e}")
                #self.scheduler.step()
        ...
        # write best.json summary
        try:
            if hasattr(self.args, 'log_best_path'):
                import json
                best_summary = {
                    'best_epoch': int(best_epoch),
                    'watch_metric': self.watch_metric,
                    'best_score': float(best_res.get(self.watch_metric, 0.0)),
                    'metrics': best_res
                }
                with open(self.args.log_best_path, 'w', encoding='utf-8') as f:
                    json.dump(best_summary, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.logger.info(f"Failed to write best.json: {e}")
        
        res = self.eval(test=True)
        # 写入包含测试结果的 best.json 摘要
        try:
            if hasattr(self.args, 'log_best_path'):
                import json
                combined_summary = {
                    ...
                    'best_score': float(best_res.get(self.watch_metric, 0.0)),
                    'metrics': best_res,
                    'test_score': float(res.get(self.watch_metric, 0.0)) if isinstance(res, dict) else None,
                    'test_metrics': res if isinstance(res, dict) else None
                }
                with open(self.args.log_best_path, 'w', encoding='utf-8') as f:
                    json.dump(combined_summary, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.logger.info(f"Failed to write best.json with test metrics: {e}")
